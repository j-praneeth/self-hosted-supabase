...
BEGIN
  FOR item IN SELECT * FROM jsonb_array_elements(seed) LOOP
    ...
    -- Resolve department by code first, then fall back to name LIKE matches
    -- (the sheet labels e.g. "Pharma-PC" / "Trace-Metals" which may or may not
    -- match the configured code). Missing dept is non-fatal — we leave the
    -- employee with department_id NULL and continue, so a single missing
    -- department doesn't abort the whole seed.
    _dept_id := NULL;
    IF _dept IS NOT NULL THEN
      ...
      IF _dept_id IS NULL THEN
        SELECT id INTO _dept_id FROM public.departments
         WHERE lower(code) = lower(_dept)
            OR lower(name) = lower(_dept)
            OR lower(name) LIKE '%' || lower(_dept) || '%'
            OR (_dept = 'TL'    AND lower(name) LIKE '%trace%metal%')
            OR (_dept = 'TL-MS' AND lower(name) LIKE '%trace%ms%')
            OR (_dept = 'PC'    AND (lower(name) LIKE '%physical%characteris%' OR lower(name) LIKE '%pharma-pc%'))
            OR (_dept = 'PL'    AND (lower(name) LIKE '%pharma%lab%'           OR lower(name) LIKE '%pharma-pl%'))
            OR (_dept = 'MB'    AND lower(name) LIKE '%microbiology%')
            OR (_dept = 'SRO'   AND lower(name) LIKE '%sample%registration%')
            OR (_dept = 'PS'    AND (lower(name) LIKE '%purchase%' OR lower(code) = 'stores'))
            OR (_dept = 'QA'    AND lower(name) LIKE '%quality%')
         LIMIT 1;
      END IF;
      IF _dept_id IS NULL THEN
        RAISE NOTICE 'Seed dept % not resolved — employee % will be created with NULL department_id', _dept, _code;
      END IF;
    END IF;
